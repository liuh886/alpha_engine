...
export function PositionsTable({ positions, report }: { positions: Position[]; report?: ReportRow[] }) {
  ...
  const currentPositions = useMemo<PositionWithChange[]>(() => {
    ...
    return positions
      ...
      .map((position) => {
        ...
        const tradeStatus = position.trade_status === 'trade'
          ? 'trade'
        ...
        return {
          ...
          tradeStatus,
          tradeAction: String(position.trade_action ?? ''),
        };
      })
  ...
  const snapshotStats = useMemo(() => {
    ...
    const reportRow = report?.find((row) => row.date === currentDate);
    const turnover = finiteNumber(reportRow?.turnover);
    ...
    <div className="space-y-5">
      <Card>
        <CardHeader className="border-b pb-3">
          ...
          <CardDescription className="text-xs">Use the retained turnover history as a navigator, then inspect the portfolio snapshot and exact retained rebalance cost below.</CardDescription>
        </CardHeader>
        ...
        <CardHeader className="border-b pb-3">
          ...
          {previousDate && (
            <p className="mt-3 text-[10px] text-muted-foreground">
              Since previous snapshot: {snapshotStats.added} added · {snapshotStats.adjusted} resized · {snapshotStats.exited} exited. Cost values come from the retained trade ledger; the browser does not reconstruct them.
            </p>
          )}
        </CardHeader>
