  ...
  if (run.formalPackage) {
    ...
    return (
      <div className="space-y-4">
        ...
        <EvidenceTable rows={run.formalPackage.report as Array<Record<string, unknown>>} source="formalPackage.report" scope="accessible performance-path alternative" />
      </div>
    );
  }
...
function RobustnessEvidence({ run, section }: { run: GovernedRunSummary; section: SectionState }) {
  if (run.formalPackage) {
    return (
      <div className="space-y-4">
        <EvidenceTable rows={run.formalPackage.window_summary} source="formalPackage.window_summary" scope="walk-forward/window evidence" />
  ...
  if (run.formalPackage) {
    ...
    return (
      <div className="space-y-4">
        ...
        <EvidenceTable rows={run.formalPackage.positions as Array<Record<string, unknown>>} source="formalPackage.positions" scope="retained holdings and concentration" />
        {run.formalPackage.trades.length
          ? <EvidenceTable rows={run.formalPackage.trades} source="formalPackage.trades" scope="signal/execution changes and retained cost drag" />
        ...
        {run.formalPackage.attribution.length
          ? <EvidenceTable rows={run.formalPackage.attribution} source="formalPackage.attribution" scope="retained contribution evidence" />
