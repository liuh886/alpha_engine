      ...
      <CardContent className="pt-4">
        <div className="h-[240px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart layout="vertical" data={data} margin={{ left: 10, right: 30, top: 5, bottom: 5 }}>
              ...
              <YAxis dataKey="name" type="category" width={140} tick={{ fontSize: 10 }} interval={0} axisLine={false} tickLine={false} />
              <Tooltip cursor={{ fill: 'rgba(0,0,0,0.03)' }} formatter={(v: number) => [`${(v * 100).toFixed(1)}%`, 'Weight']} />
        ...
        {data[0] && (
          <p className="mt-3 text-xs text-muted-foreground">
            Top feature: <span className="font-medium text-foreground">{data[0].name}</span> ({((data[0].value) * 100).toFixed(1)}%)
          </p>
        )}
      </CardContent>
    </Card>
