...
export function ComparePage({ models }: { models: ModelData[] }) {
  ...
  if (models.length === 0) {
    return <div className="research-empty-state"><GitCompareArrows className="mx-auto h-8 w-8 text-muted-foreground" /><h2 className="mt-4 text-lg font-semibold">No comparable records</h2><p className="mt-2 text-sm text-muted-foreground">The active research bundle does not declare any model or strategy records.</p></div>;
  }

  return (
    <div className="mx-auto max-w-[1500px] space-y-6 pb-12">
      <section><p className="text-xs font-bold uppercase tracking-[0.18em] text-primary">Like-for-like review</p><h2 className="mt-2 text-2xl font-black tracking-tight">Compare formal evidence</h2><p className="mt-2 max-w-3xl text-sm leading-relaxed text-muted-foreground">Select up to {MAX_COMPARE} records. Metrics are descriptive unless market, benchmark, window, snapshot and formal cost contract are aligned.</p></section>

      <Card><CardHeader className="pb-3"><CardTitle className="text-sm">Evidence records</CardTitle></CardHeader><CardContent className="flex flex-wrap gap-2">{models.map((model) => {
      ...
      {selected.length < 2 ? <div className="rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground">Select at least two evidence records to compare.</div> : <>
