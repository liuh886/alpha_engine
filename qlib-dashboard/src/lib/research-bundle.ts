...
export function validateBundleManifest(value: unknown): ResearchBundleManifest {
  ...
  if (manifest.research_only !== true || manifest.trade_ready !== false) {
    throw new Error('Bundle research boundary is missing or invalid.');
  }
  ...
  for (const artifact of manifest.artifacts) {
    ...
    if (!/^[a-f0-9]{64}$/.test(String(artifact.sha256))) {
      throw new Error(`Invalid artifact digest: ${artifact.path}`);
    }
  }
...
async function verifyArtifact(source: BundleFileSource, artifact: BundleArtifact): Promise<Blob> {
  ...
  if (blob.size !== artifact.byte_size) {
    throw new Error(`Artifact size mismatch: ${artifact.path}`);
  }
  if ((await sha256(blob)) !== artifact.sha256) {
    throw new Error(`Artifact digest mismatch: ${artifact.path}`);
  }
  return blob;
}

export async function openResearchBundle(
  source: BundleFileSource,
  options: { verifyAll?: boolean } = {},
): Promise<OpenedResearchBundle> {
  ...
  if (!modelsArtifact || !staticManifestArtifact) {
    throw new Error('Bundle does not contain required model and export indexes.');
  }
...
export class DirectoryBundleSource implements BundleFileSource {
  readonly label: string;
  constructor(readonly handle: DirectoryHandleLike) {
    this.label = handle.name || 'Local results folder';
  }
...
export class ZipBundleSource implements BundleFileSource {
  readonly label: string;
  private constructor(label: string, private readonly entries: Map<string, Blob>) {
    this.label = label;
  }
  ...
  static async fromFile(file: File): Promise<ZipBundleSource> {
    ...
    for (const entry of rawEntries) {
      ...
      entries.set(safePath(entry.name.slice(prefix.length)), new Blob([content]));
    }
...
export function getActiveResearchBundle(): OpenedResearchBundle | null {
  return activeBundle;
}
