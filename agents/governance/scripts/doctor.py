...
def check_metadata_integrity(fix=False):
    ...
    if not db_path.exists():
        ...
        print("      -> Don't panic! It hasn't been created yet. Run `make data` or click `Train` in the UI to initialize it.")
        return False
    ...
    try:
        ...
        try:
            ...
            for run in runs:
                ...
                if not found_mlflow:
                    print(f"  [!] SQLite record {run['tag']} ({run_id}) has NO matching MLflow artifact.")
                    errors += 1
                    if fix:
...
def main():
    ...
    parser.add_argument("--fix", action="store_true", help="Attempt to fix metadata inconsistencies")
    ...
    if dep_ok and integ_ok:
        ...
        if not args.fix:
            print("          -> Try running `python scripts/doctor.py --fix` to resolve minor inconsistencies.")
