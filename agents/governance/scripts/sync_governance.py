...
def sync_scripts_readme():
    ...
    new_content = re.sub(r"## Supported entrypoints\n.*?(?=\n## Utilities)", supported_text, content, flags=re.DOTALL)
    new_content = re.sub(r"## Utilities \(use as needed\)\n.*?(?=\n## Legacy)", utility_text, new_content, flags=re.DOTALL)
...
def sync_workflow():
    ...
    new_content = re.sub(r"### Step 3 - Execute Supported Entrypoint .*?(?=\n\nExample:)", mapping_text, new_content, flags=re.DOTALL)
