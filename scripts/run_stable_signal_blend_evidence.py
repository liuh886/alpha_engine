"""Generate rolling evidence for ranker + inverted momentum blend candidates."""
...
from src.research.stable_signal_blend import build_blend_candidates, default_blend_weights
...
def _selected_ranker_grid():
    candidates = {candidate.name: candidate for candidate in build_ranker_calibration_grid()}
    missing = TARGET_RANKER_NAMES - set(candidates)
    if missing:
        raise ValueError(f"target ranker grid candidates missing: {sorted(missing)}")
...
def run(root: Path, *, first_test_year: int, last_test_year: int) -> dict[str, object]:
    ...
    calendar = pd.DatetimeIndex(
        D.calendar(start_time=str(session["train_start"]), end_time=str(session["test_end"]), freq="day")
    )
    if calendar.empty:
    ...
    if not windows:
        raise ValueError("no complete rolling windows are covered by the available Qlib range")
    ...
    for window in windows:
        ...
        for candidate in ranker_grid:
            cols = [expression_columns[expression] for expression in candidate.feature_group.expressions]
            ...
            ranker_scores[candidate.name] = predict_lgbm_daily_ranker(ranker, features_test.loc[:, cols])

        candidates = {
            **ranker_scores,
            **build_blend_candidates(ranker_scores, baseline, weights=default_blend_weights()),
