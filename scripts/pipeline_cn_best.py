...
import json, sys, time, uuid, pickle
...
def stage_data():
    ...
    extra = ["$close/Ref($close,5)-1", "$close/Ref($close,10)-1", "$close/Ref($close,20)-1",
    ...
    # Split
    tr = (X_all.index.get_level_values("datetime") >= TRAIN_START) & (X_all.index.get_level_values("datetime") <= TRAIN_END)
    va = (X_all.index.get_level_values("datetime") >= VALID_START) & (X_all.index.get_level_values("datetime") <= VALID_END)
    te = (X_all.index.get_level_values("datetime") >= TEST_START) & (X_all.index.get_level_values("datetime") <= TEST_END)
...
def stage_train(Xtr, ytr, Xva, yva):
    ...
    _s = lambda c: (str(c).replace("$","D").replace("/","_d_").replace("(","L")
                     .replace(")","R").replace(",","_").replace(" ","_").replace("-","neg"))
    ...
    params = {"objective": "regression", "metric": "l2", "learning_rate": 0.05,
              "max_depth": 10, "num_leaves": 128, "feature_fraction": 0.8879,
              "bagging_fraction": 0.8789, "lambda_l1": 1.0, "lambda_l2": 1.0,
              "num_threads": 20, "verbosity": -1, "min_data_in_leaf": 20}
    booster = lgb.train(params, tr_d, num_boost_round=N_ESTIMATORS,
                        valid_sets=[va_d],
...
def stage_validate(booster, Xte, fnames, close_all, symbols):
    ...
    if spreads:
        ...
        tb_result = {
            ...
            "spread_ir": float(np.mean(arr) / np.std(arr) * np.sqrt(252 / REBALANCE_DAYS)) if np.std(arr) > 1e-10 else 0.0,
        }
        print(f"[VALIDATE] TOP/BOTTOM: annual_spread={tb_result['annualized_spread']:+.1%} "
    ...
    print(f"[VALIDATE] WF: mean_IC={wf.mean_ic:.4f} IR={wf.ic_ir:.2f} consistency={wf.consistency_score:.0%}")
...
def stage_backtest(predictions, symbols):
    ...
    # Vectorized backtest
    vec = run_vectorized_backtest(predictions, real_returns, bench,
    ...
    print(f"[BACKTEST] Grade+Regime: excess={grade.excess_return:.2%} sharpe={grade.sharpe_ratio:.2f} mdd={grade.max_drawdown:.2%}")
...
def stage_register(booster, fnames, predictions, vec, grade, tb, wf):
    ...
    metrics = {
        ...
        "walk_forward": {"mean_ic": wf.mean_ic, "ic_ir": wf.ic_ir,
                         "consistency": wf.consistency_score, "n_splits": len(wf.splits)},
        "model_tag": MODEL_TAG, "market": MARKET,
        "training_period": f"{TRAIN_START}-{TRAIN_END}",
        "test_period": f"{TEST_START}-{TEST_END}",
        "config": {"negate_label": NEGATE_LABEL, "n_features": len(fnames),
                   "top_k": TOP_K, "rebalance_days": REBALANCE_DAYS, "n_estimators": N_ESTIMATORS},
        "created_at": datetime.now().isoformat(),
    }
    (artifact_dir / "metrics.json").write_text(json.dumps(metrics, indent=2, default=str))

    # Manifest
    manifest = {"artifact_id": artifact_id, "model_id": f"cn_model_{MODEL_TAG}",
                "tag": MODEL_TAG, "market": MARKET, "created_at": datetime.now().isoformat(),
                "model_type": "LightGBM", "n_features": len(fnames),
    ...
    try:
        ...
        entry = {
            "id": version_id, "tag": MODEL_TAG,
            "name": f"CN Best v1 — 6yr Alpha158 negated label",
            "market": MARKET, "model_type": "LightGBM",
            "path": str(model_path).replace("\\", "/"), "run_id": artifact_id,
            "created_at": str(datetime.now().date()), "stage": "STAGING",
            "params": {"learning_rate": 0.05, "max_depth": 10, "n_features": len(fnames),
                       "negate_label": NEGATE_LABEL, "training": "2019-2024"},
            "backtest": {"metrics": grade.to_dict()},
            "walk_forward": {"gate_passed": tb.get("positive_ratio", 0) >= 0.5,
                             "mean_ic": wf.mean_ic, "ic_ir": wf.ic_ir,
...
def stage_display(version_id, artifact_id, metrics):
    ...
    entry = {
        "id": version_id, "run_id": artifact_id,
        "name": f"CN Best v1 (6yr, negated, {REBALANCE_DAYS}d)",
        "date": str(datetime.now().date()),
        "experiment": "cn_best_pipeline",
        "market": MARKET,
        "params": {"n_features": metrics["config"].get("n_features", 0),
                   "training": "2019-2024", "negate_label": NEGATE_LABEL},
        "data": {
...
def main():
    ...
    version_id, artifact_id, metrics = stage_register(booster, fnames, predictions, vec, grade, tb, wf)
    ...
    print(f"  Walk-forward:  mean_IC={wf.mean_ic:.4f} IR={wf.ic_ir:.2f} C={wf.consistency_score:.0%}")
    print(f"  Backtest(vec): excess={vec.excess_return:.2%} sharpe={vec.sharpe_ratio:.2f}")
    print(f"  Backtest(grd): excess={grade.excess_return:.2%} sharpe={grade.sharpe_ratio:.2f} mdd={grade.max_drawdown:.2%}")
    print(f"  ─────────────────────────────────────────────")
    print(f"  Registered:    SQLite ✓  Dashboard ✓  Artifact ✓")
    print("=" * 70)
