...
def cleanup():
    ...
    conn.execute("UPDATE jobs SET status = 'failed', error = 'Stale task cleaned by system' WHERE status = 'running'")
    conn.commit()
    conn.close()
    ...
    try:
        ...
        subprocess.run(["powershell.exe", "-Command", "Get-Process python | Where-Object { $_.CommandLine -like '*update_data.py*' -or $_.CommandLine -like '*arena_settle.py*' } | Stop-Process -Force"], check=False)
