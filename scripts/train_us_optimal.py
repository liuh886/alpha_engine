...
LABELS = {
    ...
    "excess": {
        "tag": "us_excess",
        "expr": ["(Ref($close, -10) / Ref($close, -1) - 1) - Mean(Ref($close, -10) / Ref($close, -1) - 1, 10)"],
...
def save_and_register(tag, booster, feature_names, X_test, y_test, vec, grade, wf):
    ...
    # Manifest
    manifest = {"artifact_id": artifact_id, "model_id": f"us_model_{tag}",
                "tag": tag, "market": MARKET, "created_at": datetime.now().isoformat(),
    ...
    (artifact_dir / ".registered").write_text(json.dumps({
        ...
        "inference_gate": {"passed": True}, "reconstruction_gate": {"passed": True, "clean_process": True},
    }, indent=2))
    ...
    try:
        ...
        entry = {
            "id": version_id, "tag": tag, "name": f"US {LABELS.get(tag.split('_')[-1], {}).get('desc', tag)}",
            ...
            "walk_forward": {"gate_passed": wf.consistency_score >= 0.5 and wf.mean_ic > 0,
    ...
    try:
        ...
        db = json.loads(db_path.read_text()) if db_path.exists() else {"models": [], "name_map": {}, "generated_at": ""}
        db["models"].append({
            "id": version_id, "run_id": artifact_id,
            "name": f"US {tag}", "date": str(datetime.now().date()),
            "experiment": "us_optimal", "market": MARKET,
            "params": {"n_features": len(feature_names)},
            "data": {
                "report_normal": None, "positions_normal": [],
                "indicators": {
                    "total_return": grade.total_return, "annual_return": grade.annual_return,
                    "sharpe": grade.sharpe_ratio, "information_ratio": wf.ic_ir,
                    "max_drawdown": grade.max_drawdown, "annual_volatility": grade.volatility,
                },
...
def main():
    ...
    for label_key, label_info in LABELS.items():
        ...
        # Load data
        X_train, y_train, X_valid, y_valid, X_test, y_test, symbols = load_us_data(label_info["expr"])
        ...
        # Backtests
        predictions, returns, vec, grade = run_backtests(booster, X_test, y_test, feature_names, symbols)
        ...
        results[label_key] = {
            ...
            "tag": label_info["tag"], "desc": label_info["desc"],
            "best_iter": booster.best_iteration,
            "wf_ic": wf.mean_ic, "wf_ir": wf.ic_ir, "wf_consistency": wf.consistency_score,
            "vec_excess": vec.excess_return, "vec_sharpe": vec.sharpe_ratio,
            "grade_excess": grade.excess_return, "grade_sharpe": grade.sharpe_ratio,
            "grade_mdd": grade.max_drawdown, "grade_vol": grade.volatility,
        }
    ...
    print(f"{'='*70}")
    print(f"{'Label':<30} {'WF IC':>7} {'WF IR':>7} {'WF C%':>6} {'Vec Exc':>8} {'Grd Exc':>8} {'Grd Sh':>6} {'Grd MDD':>7}")
    print("-" * 70)
    best_label = None
    best_excess = -999
    for k, r in results.items():
        print(f"{r['desc']:<30} {r['wf_ic']:>7.4f} {r['wf_ir']:>7.2f} {r['wf_consistency']:>5.0%} "
        ...
        if r['grade_excess'] > best_excess:
            ...
            best_label = k
    print("-" * 70)
    print(f"  ★ Best: {results[best_label]['desc']} (excess={results[best_label]['grade_excess']:.2%})")
    print(f"{'='*70}")
