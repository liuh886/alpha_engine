...
contract and in ``src.research.us_qlib_execution_adapter``.
"""
...
def run(
    ...
    *,
    ...
    if spec.market != "us":
        raise ValueError("run_us_feature_quality_validation requires a US spec")

    dry_run_paradigm(spec, root=root, output_dir=output_dir)
    runtime = QlibUSExecutionRuntime(provider_uri=provider_uri)
    executor = partial(execute_us_qlib_plan, runtime=runtime)
    try:
