"""Run bounded CN130 XGBoost development and one frozen 2026H1 challenge.

Candidate selection uses only complete 2024H1--2025H2 windows and prioritizes
ranking-signal consistency before portfolio return. The selected candidate and
the immutable baseline are then evaluated once on 2026H1.
"""
...
DEV_SPEC = Path("configs/research_paradigms/cn_10d_xgb_optimization_dev_v1.yaml")
BASELINE_SPEC = Path("configs/research_paradigms/cn_10d_selected_pool_ranker_retest_v1.yaml")
BASELINE_NAME = (
...
def _development_candidates(run_dir: Path) -> list[dict[str, Any]]:
    ...
    for raw in stability.get("candidates", []):
        ...
        drawdown_penalty = max(0.0, -float(row["worst_drawdown"]) - 0.15)
        row["selection_score"] = (
            0.45 * float(row["mean_icir"])
            + 2.00 * float(row["mean_rank_ic"])
            + 1.00 * float(row["mean_spread"])
            + 0.35 * float(row["compounded_relative_excess_return"])
            + 0.10 * float(row["positive_spread_ratio"])
            - 1.50 * drawdown_penalty
        )
        row["eligible"] = bool(
            int(row["n_windows"]) >= 4
            and float(row["mean_icir"]) > 0.0
            and float(row["mean_rank_ic"]) > 0.0
            and float(row["mean_spread"]) > 0.0
            and float(row["positive_spread_ratio"]) >= 0.75
            and float(row["positive_excess_ratio"]) >= 0.50
        )
        rows.append(row)
...
def _decision(selected: dict[str, Any], baseline: dict[str, Any]) -> str:
    ...
    selected_rank_ic = float(selected["rank_ic"])
    baseline_rank_ic = float(baseline["rank_ic"])
    signal_ok = (
        float(selected["icir"]) > 0.0
        and selected_rank_ic > 0.0
        and float(selected["score_direction"]["top_minus_bottom_spread"]) > 0.0
    )
    rank_improved = selected_rank_ic >= baseline_rank_ic + 0.01
    return_improved = selected_excess >= baseline_excess + 0.02
    risk_not_worse = selected_dd >= baseline_dd - 0.02
    if signal_ok and risk_not_worse and (rank_improved or return_improved):
...
def run(
    ...
    *,
    provider_uri: Path,
    output_dir: Path = Path("artifacts/evidence/cn130_xgb_optimization_v1"),
) -> dict[str, Any]:
    root = root.resolve()
    output_dir = output_dir.resolve()
    dev_result = run_cn_validation(
        root,
        spec_path=DEV_SPEC,
        output_dir=output_dir / "development",
        provider_uri=provider_uri,
    )
    dev_run_dir = output_dir / "development" / "cn_10d_xgb_optimization_dev_v1"
    ...
    selected_experiment = "cn_10d_xgb_optimization_challenge_v1"
    baseline_experiment = "cn_10d_xgb_baseline_challenge_v1"
    ...
    payload = {
        ...
        "decision_rules": {
            "minimum_rank_ic_improvement": 0.01,
            "minimum_excess_improvement": 0.02,
            "maximum_drawdown_deterioration": 0.02,
            "positive_challenge_icir_rank_ic_and_spread_required": True,
        },
    }
    _write_json(output_dir / "optimization_decision.json", payload)
    return payload
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("artifacts/evidence/cn130_xgb_optimization_v1"),
    )
