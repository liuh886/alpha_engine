...
def generate_stocks():
    ...
    for sym in symbols:
        ...
        ohlcv = []
        base_price = 150.0 if sym == "AAPL" else 800.0 if sym == "NVDA" else 3500.0 if sym == "000300.SH" else 1800.0
...
def generate_arenas():
    ...
    us_lb = {
        ...
        "leaderboard": [
            {"rank": 1, "participant_name": "LGBM Alpha Spec 1", "nav": 1.45, "daily_return": 0.012, "drawdown": 0.05, "run_id": "LGBM_v1", "edge_explanation": "Maintains superior risk-adjusted returns during high-volatility regimes due to deep momentum feature reliance."},
            {"rank": 2, "participant_name": "Transformer Q-Learn", "nav": 1.32, "daily_return": -0.005, "drawdown": 0.08, "run_id": "TF_v1"}
        ]
    }
    ...
    cn_lb = {
        "date": datetime.datetime.now().strftime("%Y-%m-%d"),
        "leaderboard": [
            {"rank": 1, "participant_name": "CN XGBoost Base", "nav": 1.15, "daily_return": 0.008, "drawdown": 0.03, "run_id": "XGB_v1", "edge_explanation": "Exhibits exceptional Alpha capture during index rebalancing events, leveraging unique sector rotation factors."}
        ]
    }
    ...
    quality = {
        "quality": {
            "summary": {
                ...
                "markets": {
                    "us": {"instruments": 8500, "instrument_end_max": "2024-05-12", "stale_instruments": 12, "csv_parse_errors": 0},
                    "cn": {"instruments": 5000, "instrument_end_max": "2024-05-12", "stale_instruments": 5, "csv_parse_errors": 0}
                },
                "warnings": []
            }
        }
...
def generate_models():
    models = {
        "versions": [
            {
                "id": "model_us_1", "tag": "US Transformer Base", "name": "US Transformer Base", "market": "us", "model_type": "transformer",
                "run_id": "LGBM_v1", "created_at": "2024-05-01", "description": "RECOMMENDED",
                "metrics_json": {"annualized_return": 0.25, "sharpe": 1.8},
                "params_json": {"learning_rate": 0.01, "max_depth": 6, "benchmark": "SPY"}
            },
            {
                "id": "model_cn_1", "tag": "CN XGBoost Base", "name": "CN XGBoost Base (CSI 300)", "market": "cn", "model_type": "xgboost",
                "run_id": "XGB_v1", "created_at": "2024-05-05", "description": "STAGING",
...
def generate_reports():
    reports = {
        "reports": [
            {
                "id": "rep_101", "type": "arena_daily", "ref_id": "arena-us-main", "date": datetime.datetime.now().strftime("%Y-%m-%d"),
                "paths": {"html": "/artifacts/reports/arena_us_latest.html"}, "meta": {"market": "us"}
            },
            {
                "id": "rep_102", "type": "backtest", "ref_id": "LGBM_v1", "date": "2024-05-01",
                "paths": {"html": "/artifacts/reports/backtest_lgbm_v1.html"}, "meta": {"market": "us", "model_type": "lgbm"}
            }
        ]
    }
        ...
        "models": [
            {
                ...
                "data": {
                    ...
                    "feature_importance": {"Momentum_20d": 0.35, "RSI_14": 0.22, "Volatility_30d": 0.18, "MACD_Signal": 0.15, "Volume_Trend": 0.10}
                }
            },
            {
                "id": "model_cn_1",
                ...
                "data": {
                    ...
                    "feature_importance": {"Price_Reversion_5d": 0.40, "Liquidity_Shock": 0.25, "Momentum_10d": 0.15, "Beta_Sensitivity": 0.12, "Sector_Rotation": 0.08}
                }
            }
        ]
    }
