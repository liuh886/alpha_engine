...
The former rolling-lab implementation independently loaded session_config.json,
selected features, zero-filled missing values, chose model calibration and Top-K,
and wrote a separate evidence tree. Those semantics now belong to the US YAML
...
def _reject_legacy_window_overrides(
    *,
    ...
    if active:
        ...
        raise ValueError(
            "Legacy rolling-runner window overrides are no longer accepted. "
            "Select or edit a YAML research spec so the declared and executed "
            f"contracts remain identical: {rendered}"
        )
...
def run(
    ...
    *,
    ...
    """Delegate to the canonical US runner without owning research semantics."""
