#!/usr/bin/env python3
"""Resolve and build the governed formal US x1.3 current target."""
...
from src.research.us_x1_3_current_target import MODEL_ID, score_us_x1_3_current_target
...
class USX13CurrentTargetCommandError(ValueError):
    """Raised when the active formal US x1.3 bundle cannot be resolved exactly."""


def _read(path: Path) -> dict[str, Any]:
    value = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise USX13CurrentTargetCommandError(f"expected JSON object: {path}")
    return value
...
def resolve_formal_bundle(formal_root: Path) -> tuple[Path, Path]:
    """Resolve the single accepted formal US x1.3 manifest and portfolio section."""
    ...
    if len(records) != 1:
        raise USX13CurrentTargetCommandError(
    ...
    if not manifest.is_file() or _sha256(manifest) != record.get("manifest_sha256"):
        raise USX13CurrentTargetCommandError("US x1.3 formal manifest identity mismatch")
    ...
    if (
        ...
        raise USX13CurrentTargetCommandError("US x1.3 formal publication boundary changed")
    portfolio_rows = [
        row
    ...
    if len(portfolio_rows) != 1 or portfolio_rows[0].get("availability_status") != "available":
        raise USX13CurrentTargetCommandError("US x1.3 formal portfolio section is unavailable")
    ...
    if (
        ...
        raise USX13CurrentTargetCommandError("US x1.3 formal portfolio identity mismatch")
...
def _build(args: argparse.Namespace) -> int:
    ...
    signal = score_us_x1_3_current_target(
...
def main() -> int:
    ...
    due.add_argument(
        "--ledger-dir",
        type=Path,
        default=Path("data/research/strategy_signal_ledgers/us_x1_3"),
    )
