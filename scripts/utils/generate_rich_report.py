...
def run_simulation(market, model, top_k=10, initial_capital=100000000):
    ...
    if not universe_file.exists(): return None
    ...
    daily_univ_ret = full_df.groupby(level=0).apply(lambda x: x["close"].mean()).pct_change().fillna(0)
    bench_equity = 1.0
    ...
    for d in dates:
        ...
        for t, pos in positions.items():
            price = day_data[day_data["instrument"]==t]["close"].values[0] if t in day_data["instrument"].values else pos["entry_price"]
        ...
        for t in target_tickers:
            if t not in positions:
                ...
                if cash >= (cost + fee):
                    cash -= (cost + fee)
                    positions[t] = {"shares": shares, "entry_price": price, "entry_date": d.strftime("%Y-%m-%d")}
    ...
    return {
        "meta": {"market": market.upper(), "generated_at": str(datetime.datetime.now().strftime('%Y-%m-%d %H:%M'))},
