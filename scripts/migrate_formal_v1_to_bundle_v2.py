"""Migrate the three accepted formal v1 packages into Model Run Bundle v2.

The migration is a deterministic evidence projection. It does not rerun models,
recompute absent ledgers, reopen model selection, or alter the immutable v1
source packages. Every retained row is copied into a named section and lineage
binds the exact source-package SHA-256.
"""
...
def _unavailable_reason(package: Mapping[str, Any], key: str) -> str:
    ...
    return f"The accepted formal v1 source did not retain {key}; migration does not reconstruct it."
...
def migrate(source_root: Path, output_root: Path) -> dict[str, Any]:
    ...
    if set(source_paths) != set(MODEL_MAP):
        raise FormalV1MigrationError("formal v1 catalog must contain exactly three accepted baselines")
