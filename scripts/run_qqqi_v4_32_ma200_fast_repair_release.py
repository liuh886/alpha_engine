#!/usr/bin/env python3
"""Run the frozen v4.32 MA200 slow-entry / fast-repair-release experiment."""
...
def _scope_run(
    ...
    *,
    ...
    headline, results, diagnostics = run_v4_32_comparison(
        bars,
        bridge_contract,
        fear_greed,
...
def _decision(gate: Mapping[str, Any], actual: Mapping[str, Any], proxy: Mapping[str, Any]) -> str:
    if gate["retrospective_v4_3_candidate_supported"]:
        return "v4_32_retrospective_v4_3_candidate_supported"
    ...
    if actual_dd > 0.0 and proxy_dd > 0.0:
        return "fast_release_defense_promising_gate_failed"
    return "v4_32_fast_repair_release_not_supported"
...
def _report(
    ...
    names = {
        ...
        GUARD: "v4.2 + MA200 bear defense / fast release",
        JOINT: "v4.27 + MA200 bear defense / fast release",
    }
    lines = [
        "# QQQ v4.32 MA200 慢熊防守 + Fast Repair Release", "",
        f"**Decision:** `{decision}`", "",
        "Entry is unchanged from v4.31: falling existing QQQ SMA(200). Release reuses frozen v4.2 repair semantics; no threshold or weight is changed.",
    ]
    for title, scope in (("实际 QQQI / SGOV 产品窗口", actual), ("QQQ / BIL 长机制窗口", proxy)):
        lines.extend([
            "", f"## {title}", "",
            "| 模型 | CAGR | Max DD | Sharpe | Sortino | Calmar | Turnover | Guard sessions | Repair releases |",
            "|---|---:|---:|---:|---:|---:|---:|---:|---:|",
        ])
        for row in scope["headline_metrics"]:
            guard_sessions = row.get("guard_sessions", 0)
            repair_releases = row.get("repair_release_sessions", 0)
            if pd.isna(guard_sessions):
                guard_sessions = 0
            if pd.isna(repair_releases):
                repair_releases = 0
            lines.append(
                "| " + " | ".join(
                    [
    ...
    lines.extend([
        ...
        "",
        "## Promotion boundary", "",
        "- Passing this gate creates only a retrospective v4.3 candidate.",
        "- v4.2 remains the formal baseline and alert source.",
        "- Formal v4.3 requires a separate prospective/shadow period that was not used to design v4.27/v4.31/v4.32, followed by an explicit promotion decision.",
        "- No MA, repair, allocation, panic or persistence tuning is authorized after this result.",
    ])
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "configs/research_paradigms/qqqi_qqq_tqqq_v4_32_ma200_fast_repair_release.yaml"
        ),
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("artifacts/evidence/qqqi_qqq_tqqq_v4_32_ma200_fast_repair_release"),
    )
