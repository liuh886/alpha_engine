...
def save_name_map(data):
    ...
    us_data = {
        k: v for k, v in data.items() if not k.isdigit() and not k.endswith(".HK") and len(k) < 10
    }
    cn_data = {k: v for k, v in data.items() if k.isdigit()}
    hk_data = {k: v for k, v in data.items() if k.endswith(".HK")}
    other_data = {
        k: v for k, v in data.items() if k not in us_data and k not in cn_data and k not in hk_data
    }
