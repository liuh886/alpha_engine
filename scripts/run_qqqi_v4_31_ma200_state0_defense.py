#!/usr/bin/env python3
"""Run the frozen v4.31 falling-MA200 state-0 defensive experiment."""
...
def _scope_run(
    ...
    *,
    ...
    headline, results, diagnostics = run_ma200_state0_comparison(
        bars,
        bridge_contract,
        fear_greed,
...
def _gate(
    ...
    proxy_guard_years = int(pj.get("ma200_guard_years", 0))
...
def _decision(gate: Mapping[str, Any], actual: Mapping[str, Any], proxy: Mapping[str, Any]) -> str:
    if gate["v4_3_candidate_supported"]:
        return "v4_31_ma200_joint_v4_3_candidate_supported"
    ...
    if actual_dd > 0.0 and proxy_dd > 0.0:
        return "ma200_drawdown_selector_promising_joint_gate_failed"
    return "v4_31_ma200_state0_defense_not_supported"
...
def _report(
    ...
    names = {
        ...
        GUARD: "v4.2 + falling-MA200 defense",
        JOINT: "v4.27 + falling-MA200 defense",
    }
    lines = [
        "# QQQ v4.31 Falling-MA200 state-0 二级防守研究", "",
        f"**Decision:** `{decision}`", "",
        "Selector: existing v4.2 SMA(200) is falling at signal close; execution next open. No extra lookback or slope threshold.",
    ]
    for title, scope in (("实际 QQQI / SGOV 产品窗口", actual), ("QQQ / BIL 长机制窗口", proxy)):
        ...
        for row in scope["headline_metrics"]:
            guard_sessions = row.get("ma200_guard_sessions", 0)
    ...
    lines.extend([
        ...
        "- No MA window/slope/confirmation/allocation search is authorized after this result.",
    ])
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "configs/research_paradigms/qqqi_qqq_tqqq_v4_31_ma200_state0_defense_research.yaml"
        ),
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("artifacts/evidence/qqqi_qqq_tqqq_v4_31_ma200_state0_defense_research"),
    )
