...
def print_results(results):
    ...
    print(
        f"  Period: {TEST_START} to {TEST_END} | Alpha158+绝对收益 | TopK≈{TOP_K} | {REBALANCE_DAYS}d rebal | {COST_BPS}bps cost"
    )
    print("=" * len(hdr))
    print(hdr)
    print("-" * len(hdr))
    ...
    for k, v in results.items():
        ...
        print(
            f"{v['desc']:<38} {r.excess_return:>7.2%} {r.total_return:>7.2%} {r.benchmark_return:>7.2%} "
...
def main():
    ...
    logger.info(
        "Data alignment",
        pred_dates_n=len(pred_dates),
        ret_dates_n=len(ret_dates),
        bench_dates_n=len(bench_dates),
        common_n=len(common_pr),
        pred_sample=str(pred_dates[:3]),
        ret_sample=str(ret_dates[:3]),
