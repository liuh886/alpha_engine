...
def load_artifact_bundle_run_data(artifact_dir: Path) -> dict:
    ...
    # Try loading report_normal / positions from artifacts subdirectory
    # (generated by rebacktest_artifacts.py or full pipeline runs)
    report_path = artifact_dir / "artifacts" / "report_normal_1day.pkl"
    if report_path.exists():
        try:
            report_df = pd.read_pickle(report_path)
            run_data["report_normal"] = json.loads(
                report_df.to_json(orient="split", date_format="iso")
            )
        except Exception as e:
            print(f"  Warning: Failed to load report_normal_1day.pkl: {e}")

    positions_path = artifact_dir / "artifacts" / "positions_normal_1day.pkl"
    if positions_path.exists():
        try:
            pos_obj = pd.read_pickle(positions_path)
            run_data["positions_normal"] = parse_positions(pos_obj)
        except Exception:
            pass
