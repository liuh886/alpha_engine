...
def parse_positions(positions_obj):
    ...
    for dt, pos_data in positions_obj.items():
        ...
        for inst, info in pos_dict.items():
            if inst in ["cash", "now_account_value"]: continue
...
def format_run_name(market: str, date_str: str, run_id: str, params: dict, strategy_profile: dict) -> str:
                ...
                try:
                    with open(p, "rb") as f:
                        ...
                        if isinstance(s, pd.Series):
                            sig_analysis[key] = {dt.strftime("%Y-%m-%d"): float(v) for dt, v in s.items()}
                except:
                    pass
...
def compute_benchmark_returns(dates: list[str], symbol: str, provider_uri: str):
    ...
    try:
        ...
        if qlib.init.__module__ == 'qlib':
            cfg = build_qlib_init_cfg({}, market="us" if symbol == "QQQ" else "cn", provider_uri_default=provider_uri)
            safe_qlib_init(cfg)
...
def upsert_model_registry_to_metadata_db(model_entry: dict = None, db_path: Path = None, model_list_path: Path = None):
    """
    Handle both single entry upsert and batch upsert from model_list.yaml (for tests).
    """
    if model_list_path and db_path:
        if not model_list_path.exists(): return 0
        try:
...
def upsert_equity_curves_to_metadata_db(run_id_or_data: str | dict, market: str = None, report_normal: list = None, db_path: Path = None):
    """
    ...
    if isinstance(run_id_or_data, str) and market and report_normal and db_path:
        try:
            BacktestEquityCurveIndex(db_path=db_path).upsert_from_report_normal_json(run_id_or_data, report_normal)
            return 1
        except: return 0
    return 0
...
def build_db():
    ...
    for v in versions:
        ...
        if run_id:
            for m_dir in mlruns_dirs:
                ...
                for exp_dir in m_dir.iterdir():
                    if not exp_dir.is_dir() or exp_dir.name in ["0", ".trash"]: continue
                    cand = exp_dir / run_id
                    if cand.exists():
                        run_path = cand
                        break
        ...
        if v.get("params_json"):
            try: params = json.loads(v["params_json"])
            except: pass
        ...
        if report_normal:
            ...
            if dates:
                ...
                if cache_key not in benchmarks_cache:
                    benchmarks_cache[cache_key] = compute_benchmark_returns(dates, bench_symbol, params.get("provider_uri", "data/watchlist"))
        ...
        model_entry = {
            "id": v["id"],
            "run_id": run_id,
            "name": format_run_name(v["market"], v["created_at"], run_id or v["id"], params, strategy_profile),
            "date": v["created_at"],
            "experiment": "workflow",
            "market": v["market"],
            "params": params,
    ...
    print(f"Successfully wrote {len(enriched_models)} models with full backtest data to {dashboard_db_path}")
