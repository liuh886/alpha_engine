#!/usr/bin/env python3
"""Run the frozen QQQ downside-volatility leverage-veto experiment."""
...
from src.research.downside_volatility_veto_experiment import (
    run_downside_volatility_veto_comparison,
)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "configs/research_paradigms/qqq_tqqq_downside_vol_veto_v4_2.yaml"
        ),
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("artifacts/evidence/qqq_tqqq_downside_vol_veto_v4_2"),
    )
    parser.add_argument(
    ...
    metrics, results, prepared, diagnostics, tables = (
        run_downside_volatility_veto_comparison(bars, contract)
    )
    ...
    challenger = results["attack_downside_volatility_v4_2_75"].metrics
    summary = {
        ...
        "interpretation_guardrails": [
            "The only new factor is the frozen ten-session QQQ downside-volatility stress flag.",
            "QQQ defense, price repair, VIX, VXN and 75% TQQQ weight remain unchanged.",
            "The factor affects only leverage entry and existing-leverage exit.",
            "No lookback, quantile, persistence or definition grid is tested.",
        ],
    }
    ...
    record = {
        ...
        "contract": {
            ...
            "base_contract_sha256": _sha256(base_contract_path),
            "factor": "QQQ_realized_downside_volatility_veto",
