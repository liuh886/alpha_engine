...
def collect_profile_warnings(profile: dict, market: str) -> list[str]:
    """
    Return non-fatal warnings about profile fields that are currently ignored or only partially supported.
    The compiler should never fail hard on these; it should surface them as warnings.
    """
    warnings: list[str] = []
    ...
    universe = profile.get("universe", {}) if isinstance(profile, dict) else {}

    # meta.market is informational; compilation is driven by CLI --market.
    ...
    # model.features is informational when feature_pack=alpha158; we use Alpha158DL + extra_features.
...
def apply_profile_to_config(profile: dict, cfg: dict, market: str) -> dict:
    ...
    if any(x is not None for x in [rebalance_steps, min_hold_days, sell_on_ma, sell_rank_threshold]):
        ...
        strat_kwargs.pop("n_drop", None)
        # If the workflow template previously used WeeklyQuantRatingStrategy, clear its kwargs.
        for key in [
            "universe_size",
...
def main():
    ...
    if not profile:
        print("No strategy_profile.json found; skipping compile.")
        return

    market = args.market or profile.get("meta", {}).get("market", "")
    if not market:
        print("No market specified; skipping compile.")
        return
    market = market.lower()

    for w in collect_profile_warnings(profile, market=market):
        print(f"[profile warning] {w}")

    model_class = (profile.get("model", {}).get("class") or "").lower()
    if "linear" in model_class:
        config_path = ROOT / "configs" / f"{market}_workflow.yaml"
    else:
        config_path = ROOT / "configs" / f"{market}_lgbm_workflow.yaml"

    if not config_path.exists():
        print(f"Config template missing: {config_path}")
        return

    with open(config_path, encoding="utf-8") as f:
        cfg = yaml.safe_load(f) or {}

    cfg = apply_profile_to_config(profile, cfg, market)
    ...
    maybe_update_watchlist(profile, market)
    print(f"Updated workflow config: {config_path}")
