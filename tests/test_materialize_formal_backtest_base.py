...
from scripts.materialize_formal_backtest_base import (
    FormalBaseMaterializationError,
    materialize,
)
...
def _fixture(tmp_path: Path) -> tuple[Path, Path, str]:
    ...
    payloads = {
        "us_x1_1": b'{"model_id":"us_x1_1","evidence_cutoff":"2025-12-31"}\n',
        "cn_x1_0": b'{"model_id":"cn_x1_0","evidence_cutoff":"2026-06-15"}\n',
    }
    models: dict[str, dict[str, str]] = {}
    for model_id, payload in payloads.items():
        path = source / f"{model_id}.json"
...
def test_materializes_exact_pinned_commit_bytes(tmp_path: Path) -> None:
    ...
    assert json.loads((output / "us_x1_1.json").read_text())["evidence_cutoff"] == (
        "2025-12-31"
    )
    assert json.loads((output / "cn_x1_0.json").read_text())["evidence_cutoff"] == (
        "2026-06-15"
    )
...
def test_rejects_digest_mismatch(tmp_path: Path) -> None:
    ...
    payload["models"]["us_x1_1"]["sha256"] = "0" * 64
