...
def test_cn_accepted_precision_accepts_same_published_decimal() -> None:
    comparison = _compare_rows_at_accepted_precision(
        [{"period_return": 0.01942118706, "benchmark_return": -0.02366060168, "breadth_value": 0.2846153846}],
        [{"period_return": 0.019421187062499998, "benchmark_return": -0.02366060168368111, "breadth_value": 0.2846153846153846}],
    )
    assert comparison["exact"] is True
    assert comparison["comparison_semantics"] == "accepted_frozen_csv_serializer_or_full_precision"
...
def test_committed_cn_frozen_score_cross_sections_reproduce_sector_selection() -> None:
    universe = yaml.safe_load(
        Path("configs/research_universes/cn_selected_equities_v3.yaml").read_text(encoding="utf-8")
    )
    symbols = [str(value).zfill(6) for value in universe["symbols"]]
    ledger, identity = _load_cn_frozen_score_cross_sections(Path.cwd(), symbols=symbols)
    ...
    assert identity["source"]["source_sha256"] == "8dbd8bc437f1092f5d3ae296892cd56c463d2c96b680fdf0601f6ef508eecdd5"
...
def test_qqq_current_source_replay_ignores_restated_historical_observations() -> None:
    ...
    assert comparison["exact"] is True
    assert comparison["authority"]["historical_economic_authority"] == "accepted_formal_prefix"
...
def test_cn_regime_portfolio_respects_continuation_weights() -> None:
    ...
    ledger = pd.DataFrame([{"window": "2026H2_PARTIAL", "datetime": date, "instrument": "000001"}])
...
def test_active_rules_replay_builds_governed_inputs_before_verification(monkeypatch, tmp_path: Path) -> None:
    formal = tmp_path / "formal"
    formal.mkdir()
    (formal / "qqqi_qqq_tqqq_v4_3.json").write_text(
        json.dumps({"model_id": "qqqi_qqq_tqqq_v4_3", "evidence_cutoff": "2026-08-10"}),
        encoding="utf-8",
    )
    (formal / "cn_x1_1.json").write_text(json.dumps({"model_id": "cn_x1_1"}), encoding="utf-8")
    ...
    def fake_build_etf_reference_bundle(**kwargs):
        ...
        return {"strategy_data_ready": True, "professional_source_ready": True}
    ...
    monkeypatch.setattr(qqq_replay, "build_etf_reference_bundle", fake_build_etf_reference_bundle)
    ...
    assert receipt["decision"] == "exact_replay"
    assert calls == {"qqq_end": "2026-08-10", "cn_window": "2026H2_PARTIAL", "cn_batch": "r0r1"}
