...
def test_live_delivery_state_is_not_embedded_in_byd_preview() -> None:
    catalog = _read(NATIVE / "catalog.json")
    record = next(row for row in catalog["records"] if row["model_version_id"] == BYD_V13)
    run_dir = (NATIVE / record["manifest_path"]).parent
    payload = json.dumps(
        {
            "portfolio": _read(run_dir / "portfolio.json"),
...
def test_sync_promotes_persisted_active_preview_set_deterministically(tmp_path: Path) -> None:
    preview = _active_preview_root(tmp_path)
    first = tmp_path / "first"
    second = tmp_path / "second"
    receipt_a = sync(FRESHNESS, first, native_root=preview, strategy_catalog=STRATEGIES)
    receipt_b = sync(FRESHNESS, second, native_root=preview, strategy_catalog=STRATEGIES)
...
def test_sync_fails_closed_when_preview_catalog_is_incomplete(tmp_path: Path) -> None:
    ...
    try:
        sync(FRESHNESS, output, native_root=incomplete, strategy_catalog=STRATEGIES)
