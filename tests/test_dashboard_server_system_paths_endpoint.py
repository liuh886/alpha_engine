...
def test_dashboard_server_system_paths_endpoint():
    httpd, t = _start_server()
    try:
        port = httpd.server_address[1]
        with urllib.request.urlopen(f"http://127.0.0.1:{port}/api/system/paths", timeout=5) as resp:
            body = resp.read().decode("utf-8")
            data = json.loads(body)
        assert data.get("ok") is True
        paths = data.get("paths") or {}
        assert "project_root" in paths
        assert "dashboard_db_path" in paths
        assert "metadata_db_path" in paths
    finally:
        httpd.shutdown()
        t.join(timeout=5)
