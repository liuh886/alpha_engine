...
Verify that attribution enforces model version / data snapshot identities,
minimum-observation policies, and regularization options in the active
...
def test_attribution_report_includes_observation_metadata():
    ...
    from src.research.factor_attribution import AttributionReport, FactorContribution
    ...
    assert payload["observation_count"] == 12
    assert payload["observation_window"] == "12 monthly periods"
    assert payload["methodology"] == "OLS"
    assert payload["n_factors"] == 1
    assert payload["model_version_id"] == "mv_abc123"
    assert payload["data_snapshot_id"] == "ds_xyz789"
    assert "Low confidence" in payload["confidence_note"]
...
def test_attribution_report_defaults_observation_fields():
    ...
    assert payload["methodology"] == "OLS"
    assert payload["n_factors"] == 0
    assert payload["model_version_id"] is None
