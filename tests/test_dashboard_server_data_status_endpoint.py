...
def test_dashboard_server_data_status_endpoint():
    httpd, t = _start_server()
    try:
        port = httpd.server_address[1]
        with urllib.request.urlopen(f"http://127.0.0.1:{port}/api/data/status", timeout=5) as resp:
            body = resp.read().decode("utf-8")
            data = json.loads(body)
        assert data.get("ok") is True
        assert "latest_calendar_day" in (data.get("data") or {})
    finally:
        httpd.shutdown()
        t.join(timeout=5)
