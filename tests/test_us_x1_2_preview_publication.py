...
def _mtm_plan(*, prospective: bool = True) -> RunExportPlan:
...
def _section_payload(plan: RunExportPlan, section_id: str) -> dict:
    section = next(row for row in plan.sections if row.section_id == section_id)
    assert isinstance(section.payload, dict)
    return section.payload


def test_us_x1_2_cutoff_mtm_keeps_equity_observation_at_evidence_cutoff(
    ...
    monkeypatch.setattr(
        "src.artifacts.us_x1_2_mtm._close_panel",
        lambda *args, **kwargs: _mtm_panel(),
    )
    ...
    performance = _section_payload(result, "performance")
    summary = _section_payload(result, "summary")
