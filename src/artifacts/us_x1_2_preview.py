"""Build the governed US x1.2 research-preview evidence bundle.
...
class USX12PreviewError(ValueError):
...
def _yaml(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise USX12PreviewError(f"expected YAML mapping: {path}")
    return payload
...
def _calibration(config: Mapping[str, Any]) -> XGBNativeCalibration:
    ...
    if any(value is None for value in fields.values()):
        raise USX12PreviewError("US x1.2 native XGBoost contract is incomplete")
...
def _symbols(root: Path, runtime: QlibUSExecutionRuntime) -> list[str]:
    ...
    if len(result) != expected or len(result) != len(set(result)):
        raise USX12PreviewError("US x1.2 universe identity is not exact")
    return result
...
def _sectors(root: Path, symbols: list[str]) -> dict[str, str]:
    ...
    if not isinstance(records, dict):
        raise USX12PreviewError("US sector classification is invalid")
    result = {
    ...
    if missing:
        raise USX12PreviewError(f"US sector classification is incomplete: {missing}")
...
def _windows(runtime: QlibUSExecutionRuntime) -> tuple[list[Any], dict[str, pd.DatetimeIndex]]:
    ...
    if tuple(value.label for value in development.selected_windows) != DEVELOPMENT_WINDOWS:
        raise USX12PreviewError("US x1.2 development-window contract changed")
...
def _fit_window(
    ...
    *,
    ...
    valid, reason = validate_no_nan_inputs(
        train_features, context=f"US x1.2 preview/{window.label}"
    )
    if not valid:
        raise USX12PreviewError(reason)
...
def _holding_end(calendar: list[str], signal_date: str) -> str:
    try:
        ...
        raise USX12PreviewError(f"signal date missing from provider calendar: {signal_date}") from exc
    if index + 10 >= len(calendar):
        raise USX12PreviewError(f"unrealized US x1.2 holding: {signal_date}")
...
def _next_due_signal(calendar: list[str], last_signal: str, cutoff: str) -> str | None:
    try:
        ...
        raise USX12PreviewError(f"last signal is absent from provider calendar: {last_signal}") from exc
    due = index + 10
...
def _fit_current_signal(
    ...
    *,
    ...
    valid, reason = validate_no_nan_inputs(
        train_features, context=f"US x1.2 current signal/{signal_date}"
    )
    if not valid:
        raise USX12PreviewError(reason)
    x_rank, y_rank, groups = prepare_ranker_frame(train_features, train_returns)
...
def _metric(metric_id: str, value: float | None, *, unit: str, direction: str, sample_count: int) -> dict[str, Any]:
    ...
    return {
        ...
        "unit": unit,
        "direction": direction,
        "estimator": "governed_us_x1_2_preview_trace" if available else None,
...
def build_plan(
    ...
    *,
    ...
    if config.get("model_id") != MODEL_ID or config.get("status") != "baseline_research_active":
        raise USX12PreviewError("US x1.2 is not the active research baseline")
    ...
    if selection.get("formal_acceptance") is not False or selection.get("research_baseline_promotion") is not True:
        raise USX12PreviewError("US x1.2 preview/formal boundary changed")
    ...
    if provider is None:
        raise USX12PreviewError("US provider manifest is unavailable")
    ...
    for window in evidence:
        ...
        for period in window.periods.sort_values("period_index").to_dict("records"):
            ...
            if len(target) != 15 or not math.isclose(sum(target.values()), 1.0, abs_tol=1e-9):
                raise USX12PreviewError(f"US x1.2 target is incomplete: {signal_date}")
    ...
    diagnostics = {
        ...
        "interpretation_notes": [
            "US x1.2 is the active research baseline and is not the accepted formal baseline.",
    ...
    lineage = {
        ...
        "formal_baseline_superseded_for_research": "us_x1_1",
