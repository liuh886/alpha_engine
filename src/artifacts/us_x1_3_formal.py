"""Promote complete governed US x1.3 Bundle v2 evidence into the formal channel."""
...
SUPERSEDED_FORMAL_MODEL = "us_x1_2"
PROMOTION_AUTHORITY = "explicit_user_direction_2026_08_12"


class USX13FormalPromotionError(ValueError):
    """Raised when the governed US x1.3 evidence cannot be promoted exactly."""
...
def _object(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise USX13FormalPromotionError(f"JSON root must be an object: {path}")
    return value
...
def _verify_source_bundle(run_dir: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise USX13FormalPromotionError("US x1.3 source bundle is not the governed preview")
    for section in manifest["sections"]:
        if section["availability_status"] != "available":
            continue
        path = run_dir / str(section["path"])
        if not path.is_file():
            raise USX13FormalPromotionError(f"source section is missing: {section['section_id']}")
        data = path.read_bytes()
        if len(data) != section["byte_size"] or hashlib.sha256(data).hexdigest() != section["sha256"]:
            raise USX13FormalPromotionError(f"source section identity mismatch: {section['section_id']}")
    return manifest
...
def _formal_metrics(value: object) -> list[dict[str, Any]]:
    if not isinstance(value, list) or not all(isinstance(row, Mapping) for row in value):
        raise USX13FormalPromotionError("US x1.3 canonical metrics are invalid")
    metrics = [copy.deepcopy(dict(row)) for row in value]
    for metric in metrics:
        if metric.get("estimator") == "governed_us_x1_3_preview_trace":
            metric["estimator"] = "governed_us_x1_3_trace"
        if metric.get("metric_id") == "ic" and metric.get("availability_status") == "not_computed":
            metric["unavailable_reason"] = (
                "The governed US x1.3 builder does not compute raw IC; Rank IC is retained."
            )
    return metrics
...
def _rewrite_diagnostics(path: Path) -> None:
    ...
    diagnostics = {
        ...
        "interpretation_notes": [
            "US x1.3 is the accepted formal research baseline by explicit user direction dated 2026-08-12.",
