"""Fail-closed portfolio evidence for the v4.24 ordinal state machine."""
...
def strategy_daily(
    ...
    *,
    ...
    for block_number, block in enumerate(
        ...
        state = fixed_state or str(block.selected_state)
    ...
    *,
    ...
    results: dict[str, StrategyResult] = {
        ...
        "xgb_ordinal_state_machine": strategy_daily(
            ...
            strategy_name="xgb_ordinal_state_machine",
        ),
    }
    for state in STATE_ORDER:
        results[f"{state}_only"] = strategy_daily(
            selected,
            bars,
            contract,
            cash_symbol=cash_symbol,
            strategy_name=f"{state}_only",
            fixed_state=state,
        )
...
def phase2_gate(
    ...
    largest_state_share = (
        float(contribution.max() / total_positive) if total_positive > 0.0 else np.nan
    )
    checks = {
    ...
    return {
        ...
        * 100.0,
        ...
        "largest_state_positive_share": largest_state_share,
    }
