...
def fetch_governed_etf_strategy_bars(
    *,
    ...
    """Load executable ETFs from a governed bundle and other references normally.

    QQQ, QQQI and TQQQ are loaded from the immutable canonical bundle when one
    is supplied. Non-executable context series such as VIX and VXN continue to
    use the existing adjusted-bar adapter path.
    """
    ...
    if bundle_dir is not None and etf_symbols:
        ...
        for symbol, frame in loaded.items():
            ...
            if local.empty:
                raise ValueError(
                    f"governed ETF bundle has no rows for {symbol} in requested range"
                )
            bars[symbol] = local
