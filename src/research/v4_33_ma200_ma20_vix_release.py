"""Final deterministic release variant for the MA200 state-0 defense line.

Slow-bear entry remains falling existing SMA(200). Release is the simplest
existing v4.2 price+volatility repair: QQQ is no longer below SMA(20), and VIX
is easing or normalized. No thresholds, windows or weights are introduced.
"""
...
GUARD = "v4_33_ma200_ma20_vix_defense"
JOINT = "v4_33_panic_repair_ma200_ma20_vix_release"


def build_ma200_ma20_vix_release_trace(daily: pd.DataFrame) -> pd.DataFrame:
    """Build falling-MA200 entry and MA20+VIX close-time release trace."""
    required = {
        "ma_long",
    ...
    trace = pd.DataFrame(
        {
            ...
            "price_repaired_at_close": price_repaired.astype(bool),
            "volatility_repaired_at_close": volatility_repaired.astype(bool),
            "fast_price_vol_repair_at_close": fast_price_vol_repair.astype(bool),
...
def run_ma20_vix_release_backtest(
    ...
    *,
    ...
    active_dates = daily.index[daily["ma200_ma20_vix_defense_active"].fillna(False)]
    ...
    metrics.update(
        {
            ...
            "price_vol_release_sessions": int(
                (
                    daily["position_state"].astype(int).eq(0)
                    & daily["fast_price_vol_repair_at_close"].shift(1, fill_value=False)
                ).sum()
            ),
        }
    )
...
def run_v4_33_comparison(
    ...
    *,
    ...
    """Run v4.2, v4.27, v4.33 guard and the frozen joint candidate."""
    ...
    for result in (baseline, panic):
        ...
        result.metrics.setdefault("price_vol_release_sessions", 0)
    ...
    if not all(
        ...
        raise AssertionError("v4.33 changed the formal v4.2 state trace")
    ...
    diagnostics = {
        ...
        "price_vol_release_sessions": int(guard.metrics["price_vol_release_sessions"]),
