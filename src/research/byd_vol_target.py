...
def compute_vol_target(common, signals, max_byd=MAX_BYD):
    ...
    byd_weight = (base * scale).clip(lower=MIN_BYD, upper=max_byd)
...
def run_candidates(
    ...
    *,
    ...
    for name, decision in decisions.items():
        ...
        financing_cost = borrowed * FINANCING_RATE / FINANCING_DAY_COUNT
            ...
            rows.append(
                {
                    ...
                    "positive_contribution_share": (
                        max(value, 0.0) / positive_total if positive_total > 0 else 0.0
                    ),
                }
            )
    return pd.DataFrame(rows)
...
def governed_result(evaluation, contributions):
    def row(model, scenario, cost_bps=PRIMARY_COST_BPS):
        ...
        if len(selected) != 1:
            raise ValueError(
                f"Expected 1 row for {model}/{scenario}/{cost_bps}, got {len(selected)}"
            )
    ...
    negative_periods = int(primary_contributions["relative_terminal_wealth"].lt(0).sum())
    max_share = (
    ...
    gates = {
        ...
        "calmar_ok": float(primary["calmar"]) >= float(baseline_primary["calmar"]),
