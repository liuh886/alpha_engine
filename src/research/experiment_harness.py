...
def load_experiment_contract(path: str | Path) -> ExperimentContract:
    """Load and validate the small contract required by the locked evaluator."""
    ...
    reporting = tuple(
        str(value) for value in windows.get("consumed_reporting_only", [])
    )
    if not selection:
    ...
    if windows.get("consumed_reporting_may_enter_selection") is not False:
        raise ValueError(
            "consumed_reporting_may_enter_selection must be explicitly false"
        )
...
def _selection_rows(
    ...
    for row in observations:
        ...
        if window in by_window:
            raise ValueError(
                f"duplicate observation for {candidate_id}/{window}/{cost_bps}bps"
            )
...
def _candidate_metrics(
    ...
    base_rows = _selection_rows(
        contract, observations, candidate_id, contract.base_cost_bps
    )
    stress_rows = _selection_rows(
        contract, observations, candidate_id, contract.stress_cost_bps
    )

    relative_excess = [float(row["relative_excess"]) for row in base_rows]
    stress_relative_excess = [
        float(row["relative_excess"]) for row in stress_rows
    ]
    ...
    return {
        ...
        "mean_rank_ic_improvement": (sum(rank_ics) / len(rank_ics)) - baseline_rank_ic,
        "dominates_factor_baselines": bool(
            metadata.get("dominates_factor_baselines", False)
        ),
...
def _gate_checks(
    ...
    return {
        ...
        "stress_compounded_relative_excess": float(
            metrics["stress_compounded_relative_excess"]
        )
        > float(thresholds["min_stress_compounded_relative_excess"]),
        "strongest_positive_window_share": float(
            metrics["strongest_positive_window_share"]
        )
...
def evaluate_experiment(
    ...
    *,
    ...
    baseline_rank_ic = sum(float(row["rank_ic"]) for row in baseline_rows) / len(
        baseline_rows
    )
    ...
    evaluated.sort(
        key=lambda row: tuple(
            _ranking_value(metric, row) for metric in contract.ranking
        ),
        reverse=True,
    )
    winner = evaluated[0]
