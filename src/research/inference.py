    ...
    if profile_path and profile_path.exists():
        try:
            ...
            with open(profile_path, encoding="utf-8") as f:
                prof = json.load(f)
                features = resolve_inference_feature_list(
    ...
    if isinstance(pred_score, pd.Series):
        ...
        if len(pred_score) == len(df_test):
            ...
            pred_df = pd.DataFrame(
                pred_score[:min_len], index=df_test.index[:min_len], columns=["score"]
            )
    else:
...
def apply_inference_guardrails(
    pred_df: pd.DataFrame, tickers: list[str], inference_date: pd.Timestamp
) -> pd.DataFrame:
    """
