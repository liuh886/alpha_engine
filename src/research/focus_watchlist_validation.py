...
def _window_frame(
    ...
    frame = price.merge(states, on="date", how="inner", validate="one_to_one")
    ...
    frame["exposure_change"] = frame["exposure"].diff().fillna(frame["exposure"])
    ...
    frame = frame.merge(qqq_returns, on="date", how="left", validate="one_to_one")
    return frame
...
def _evaluate_symbol_window(
    ...
    metrics = {
        ...
        "history_status": "full" if frame.shape[0] >= minimum_sessions else "short_history",
...
def _aggregate_book(
    ...
    for symbol, frame in symbol_frames.items():
        if frame.empty:
            continue
        part = frame[["date", "strategy_return", "qqq_return", "market_regime"]].copy()
        part["symbol"] = symbol
        part["risk_weight"] = float(risk_weights[symbol])
        pieces.append(part)
        contributions[symbol] = float(
            (part["strategy_return"].fillna(0.0) * part["risk_weight"]).sum()
        )
    if not pieces:
        return {}, {}, contributions

    long = pd.concat(pieces, ignore_index=True)
    long["weighted_return"] = long["strategy_return"] * long["risk_weight"]
    grouped = long.groupby("date", sort=True)
    book = grouped.agg(
        weighted_return=("weighted_return", "sum"),
        active_weight=("risk_weight", "sum"),
        qqq_return=("qqq_return", "first"),
        market_regime=("market_regime", "first"),
    ).reset_index()
    book["strategy_return"] = book["weighted_return"] / book["active_weight"].replace(0, np.nan)
    book = book.merge(sox_regimes, on="date", how="left", validate="one_to_one")

    aggregate = {
        "strategy_total_return_after_costs": _compound(book["strategy_return"]),
        "qqq_return": _compound(book["qqq_return"]),
        "qqq_relative_return": float(
            _compound(book["strategy_return"]) - _compound(book["qqq_return"])
        ),
...
def _gate_summary(
    development_metrics: Mapping[str, Mapping[str, Any]],
    ...
    available = [row for row in development_metrics.values() if row.get("sessions", 0) > 0]
    drawdown_values = [
        float(row["drawdown_reduction_vs_buy_hold"])
        for row in available
        if row.get("drawdown_reduction_vs_buy_hold") is not None
    ]
    hold_values = [float(row["average_holding_sessions"]) for row in available]
    trade_values = [float(row["trades_per_year"]) for row in available]
    positive_strategy_ratio = float(
        np.mean([row["strategy_total_return_after_costs"] > 0 for row in available])
    ) if available else 0.0
    positive_relative_ratio = float(
        np.mean([row["qqq_relative_return"] > 0 for row in available])
    ) if available else 0.0

    enter_means: list[float] = []
    for symbol in development_metrics:
        value = forward_metrics.get(symbol, {}).get("20d", {}).get("ENTER", {}).get("mean")
        if value is not None:
            enter_means.append(float(value))
    positive_enter_ratio = float(np.mean([value > 0 for value in enter_means])) if enter_means else 0.0

    positive_contributions = {key: max(value, 0.0) for key, value in contributions.items()}
    total_positive = sum(positive_contributions.values())
    concentration = (
        max(positive_contributions.values(), default=0.0) / total_positive
        if total_positive > 0
        else 1.0
    )
    supported_regimes = 0
    for row in regime_metrics.get("market", {}).values():
        if row["strategy_return"] >= 0 or row["relative_return"] > 0:
            supported_regimes += 1

    observed = {
        "median_drawdown_reduction_vs_buy_hold": float(np.median(drawdown_values)) if drawdown_values else None,
        "aggregate_qqq_relative_return_after_costs": aggregate.get("qqq_relative_return"),
        "positive_20d_enter_return_symbol_ratio": positive_enter_ratio,
        "maximum_single_symbol_profit_contribution": float(concentration),
        "median_trades_per_year": float(np.median(trade_values)) if trade_values else None,
        "median_average_holding_sessions": float(np.median(hold_values)) if hold_values else None,
        "positive_strategy_return_symbol_ratio": positive_strategy_ratio,
        "positive_qqq_relative_symbol_ratio": positive_relative_ratio,
        "supported_market_regimes": supported_regimes,
    }
    comparisons = {
...
def run_focus_watchlist_validation(
    *,
    ...
    sox_regimes = reference_history[reference_history["symbol"] == "SOX"][["date", "regime"]].rename(
        columns={"regime": "sox_regime"}
    )
    ...
    windows = ("development_observed", "falsification_only")
    ...
    for window_name in windows:
        ...
        if window_name == "development_observed":
    ...
    if gate_summary is None:
        raise ValueError("development gate summary was not generated")
    decision = (
        "focus_signal_independent_validation_required"
        if gate_summary["all_passed"]
    ...
    decision_payload = {
        ...
        "observed_evidence_end": (reserved_start - pd.Timedelta(days=1)).date().isoformat(),
        "development_gates": gate_summary,
    }
