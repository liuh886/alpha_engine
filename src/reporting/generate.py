...
def generate_report(market):
    ...
    if cfg_path.exists():
        try:
            ...
            logger.warning(
                "Failed to parse workflow config YAML", cfg_path=str(cfg_path), exc_info=True
            )
            workflow_cfg = {}
    ...
    try:
        ...
        if df_features.empty:
            ...
            # Apply guardrails
            top_candidates["guardrail_result"] = top_candidates.apply(
                lambda row: apply_guardrails(row), axis=1
            )
            top_candidates["passed_guardrails"] = top_candidates["guardrail_result"].apply(
                lambda x: x["passed"]
            )
    except Exception as e:
    ...
    with open(report_path, "w") as f:
        ...
        violations = top_candidates[not top_candidates["passed_guardrails"]]
        if violations.empty:
            f.write("None\n")
        else:
