...
def __getattr__(name):
    if name == "CONFIG_DIR": return get_config_dir()
    if name == "DATA_DIR": return get_data_dir()
    if name == "REPORTS_DIR": return get_reports_dir()
    if name == "SCRIPTS_DIR": return get_scripts_dir()
    if name == "ARTIFACTS_DIR": return get_artifacts_dir()
    
    if name == "MLRUNS_DIR": return get_artifacts_dir() / "mlruns"
    if name == "MODELS_DIR": return get_artifacts_dir() / "models"
    if name == "RUNS_DIR": return get_artifacts_dir() / "runs"
    if name == "PYTEST_CACHE_DIR": return get_artifacts_dir() / ".pytest_cache"
    
    if name == "ARCHIVES_DIR": return get_artifacts_dir() / "archives"
    if name == "QLIB_DEMO_DATA_DIR": return (get_artifacts_dir() / "archives") / "qlib_demo_data"
    
    if name == "DASHBOARD_DIR": return get_artifacts_dir() / "dashboard"
    if name == "DASHBOARD_DB_PATH": return get_artifacts_dir() / "dashboard" / "dashboard_db.json"
