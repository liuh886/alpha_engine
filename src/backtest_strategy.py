...
def generate_interactive_report(hist_df, benchmark, prices, ticker_profits, trade_logs, model_info=None, report_path="reports/backtest_report.html"):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            name="AI Strategy", line=dict(color='#00ff88', width=2),
    ...
    fig.add_trace(
        go.Scatter(
            x=hist_df.index, y=hist_df['benchmark_value'],
        ...
        for j, ticker in enumerate(group):
            ...
            if buys_x:
                fig.add_trace(
                    go.Scatter(
                        x=buys_x, y=buys_y,
                        mode='markers', name=f"{ticker} Buy",
                        marker=dict(symbol='triangle-up', size=10, color='green'),
                        showlegend=False, legendgroup=f"group{row_idx}",
                        hoverinfo='skip'
                    ),
                    row=row_idx, col=1
                )
            if sells_x:
                fig.add_trace(
                    go.Scatter(
                        x=sells_x, y=sells_y,
                        mode='markers', name=f"{ticker} Sell",
...
def run_backtest(
    ...
    for i, date in enumerate(trading_days):
        ...
        history.append({
            ...
            "benchmark_value": initial_cash * (1 + bench_ret.loc[:date].fillna(0).add(1).cumprod().iloc[-1] - 1),
    ...
    generate_interactive_report(hist_df, benchmark, close_prices, ticker_profits, trade_logs, model_info=model_info)
