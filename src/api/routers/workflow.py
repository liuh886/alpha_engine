...
from src.workflows.hooks import run_training_pipeline, run_rebacktest_pipeline, get_task_slug
...
def run_train_wf(payload: WorkflowRequest, background_tasks: BackgroundTasks):
    ...
    task_slug = get_task_slug("run", payload.market)
    ...
    if active:
        raise HTTPException(status_code=409, detail=f"Workflow already running: {active[0]['workflow_id']}")
...
def run_backtest_wf(payload: WorkflowRequest, background_tasks: BackgroundTasks):
    gov = GovernanceService(PROJECT_ROOT)
    task_slug = get_task_slug("rebacktest", payload.market)
...
@router.get("/status")
def list_workflows(status: Optional[str] = None, limit: int = 20):
    gov = GovernanceService(PROJECT_ROOT)
